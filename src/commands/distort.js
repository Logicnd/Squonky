...
const RealityDistortionModule = require("../modules/realityDistortion");

const DISTORTIONS = ["glitch", "matrix", "mirror"];

module.exports = {
    data: new SlashCommandBuilder()
        .setName("distort")
        .setDescription("Deploy a reality distortion effect across channels.")
        .setDMPermission(false)
        .addStringOption(o =>
            o.setName("type").setDescription("Distortion type").setRequired(true)
                .addChoices(...DISTORTIONS.map(d => ({ name: d.charAt(0).toUpperCase() + d.slice(1), value: d })))),
    name: "distort",
    description: "Deploy a reality distortion effect.",
    category: "Modules",
    ownerOnly: true,
    async execute({ ctx, interaction, message, config, logger }) {
        ...
        const type = interaction?.options?.getString("type") ?? ctx.args?.[0];

        if (!type) return ctx.reply({ embeds: [embeds.warning("Missing Type", `Valid types: ${DISTORTIONS.join(", ")}`)], ephemeral: true });

        const mod = new RealityDistortionModule(config, logger);
        const source = interaction ?? message;
        await mod.execute(source, type);
    }
};
