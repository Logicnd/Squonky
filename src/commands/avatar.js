...
module.exports = {
    data: new SlashCommandBuilder()
        .setName("avatar")
        .setDescription("Retrieve a user's avatar in full resolution.")
    ...
    async execute({ ctx, interaction }) {
        ...
        const url = target.displayAvatarURL({ size: 4096, forceStatic: false });
        return ctx.reply({ embeds: [
            embeds.info(`Avatar — ${target.tag}`, null, [{ name: "Direct Link", value: `[Open](${url})`, inline: true }])
                .setImage(url)
        ] });
    }
};
